...
void SpoutTexture::_init() {
    ...
    VisualServer::get_singleton()->connect("frame_pre_draw", this, "_receive_texture");

	//VisualServer::get_singleton()->texture_set_force_redraw_if_visible(texture, true);
}
...
bool SpoutTexture::_create_receiver(const String &name) {
	...
	// Allocate texture
	vs->texture_allocate(_texture, width, height, Image::FORMAT_RGBA8);
...
void SpoutTexture::_receive_texture() {
	...
	if (width != _width || height != _height) {
		vs->texture_set_size_override(_texture, width, height);
	...
	// Receive texture
	_receiver->ReceiveTexture(channel, width, height, tex_id, GL_TEXTURE_2D, false, fbo_id);
}
